...
gen_image_spinor_proc_ai_mode()
{
	...
	if $(ls root/bin/*.kmodel >/dev/null 2>&1 )  ; then 
		...
		cd ai_mode;
		find ./ -type f  -not -name mbface.kmodel -not -name retinaface.kmodel | xargs rm -rf ;
...
gen_image_spinor()
{
	...
	cd ${BUILD_DIR}/images/little-core/rootfs/etc/init.d/; mv doorlock_rcS rcS;
	#裁剪小核rootfs
	cd ${BUILD_DIR}/images/little-core/rootfs/; 
#	rm -rf lib/modules/;
#	rm -rf lib/libstdc++*;
...
#	rm -rf app/;
	...
	cp ${K230_SDK_ROOT}/src/reference/business_poc/doorlock/big/out/door_lock.elf ./;

...
gen_image_spinor_proc_ai_mode()
{
	...
	cp ${K230_SDK_ROOT}/src/big/kmodel/door_lock/*.kmodel  root/bin/;
	rm -rf ai_mode ai_mode.bin; mkdir -p ai_mode; 
	if $(ls root/bin/*.kmodel >/dev/null 2>&1 )  ; then 
		cp  root/bin/*.kmodel ai_mode; 
		cd ai_mode;
		find ./ -type f  -not -name mbface.kmodel -not -name retinaface.kmodel -not -name ir.kmodel -not -name depth.kmodel | xargs rm -rf ;
...
gen_image_spinor()
{
	...
	cd ${BUILD_DIR}/images/little-core/rootfs/etc/init.d/; mv doorlock_ov9286_rcS rcS;
	#裁剪小核rootfs
	cd ${BUILD_DIR}/images/little-core/rootfs/; 
	...
	cp ${K230_SDK_ROOT}/src/reference/business_poc/doorlock_ov9286/big/out/door_lock.elf ./;
	mv door_lock.elf fastboot_app.elf;
	${K230_SDK_ROOT}/toolchain/riscv64-linux-musleabi_for_x86_64-pc-linux-gnu/bin/riscv64-unknown-linux-musl-strip fastboot_app.elf;
	...
	fasb_app_size=$(du -sb fastboot_app.elf | cut -f1);cp  fastboot_app.elf ${BUILD_DIR}/images/big-core/;
	if $(ls ${K230_SDK_ROOT}/src/reference/business_poc/doorlock_ov9286/big/bin/*.bin >/dev/null 2>&1 )  ; then 
		cp ${K230_SDK_ROOT}/src/reference/business_poc/doorlock_ov9286/big/bin/*.bin ${BUILD_DIR}/images/big-core/root/bin/
	fi;
	find . -type f  -not -name init.sh -not -name H1280W720_conf.bin -not -name H1280W720_ref.bin  | xargs rm -rf ;
	echo "/bin/fastboot_app.elf /bin/retinaface.kmodel /bin/depth.kmodel /bin/ir.kmodel /bin/mbface.kmodel &" > init.sh
...
gen_image_spinand()
{
	...
	cd ${BUILD_DIR}/images/big-core/root/bin/; rm -rf * ./dpu;
	cd ${RTSMART_SRC_DIR}/userapps/; python3 ../tools/mkromfs.py ${BUILD_DIR}/images/big-core/root/  ${RTSMART_SRC_DIR}/kernel/bsp/maix3/applications/romfs.c;
	cd ${K230_SDK_ROOT};make rt-smart-kernel big-core-opensbi;
	cd "${BUILD_DIR}/images/big-core/"
	cp ${BUILD_DIR}/big/rt-smart/rtthread.* .;

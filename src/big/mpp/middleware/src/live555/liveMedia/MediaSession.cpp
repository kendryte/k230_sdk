...
if(fFlag == FLAG_RECVONLY) {
    printf("MediaSubsession::initiate()  FLAG_RECVONLY\n");
    ...
    printf("MediaSubsession::initiate()  FLAG_SENDONLY\n");
    // Create "fRTPSink" and "fReadSource":
    if (!createBackChannelObjects(useSpecialRTPoffset)) break;
    if (fRTPSink == NULL) {
  ...
  printf("MediaSubsession::initiate()  FLAG NOT EXPECTED\n");
  break;
}

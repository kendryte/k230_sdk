...
int mov_reader_getinfo(struct mov_reader_t* reader, uint32_t index, struct mov_reader_trackinfo_t *ontrack, void* param)
{
	// int i;
	uint32_t j;
	struct mov_track_t* track;
	...
	// for (i = 0; i < reader->mov.track_count; i++)
	{
		track = &reader->mov.tracks[index];
		uint32_t time_scale = reader->mov.mvhd.timescale;
		for (j = 0; j < track->stsd.entry_count && j < 1 /* only the first */; j++)
		{
			...
			{
			case MOV_VIDEO:
				if(ontrack->onvideo) ontrack->onvideo(param, track->tkhd.track_ID, time_scale, entry->object_type_indication, entry->u.visual.width, entry->u.visual.height, entry->extra_data, entry->extra_data_size);
				break;

			case MOV_AUDIO:
				if (ontrack->onaudio) ontrack->onaudio(param, track->tkhd.track_ID, time_scale, entry->object_type_indication, entry->u.audio.channelcount, entry->u.audio.samplesize, entry->u.audio.samplerate >> 16, entry->extra_data, entry->extra_data_size);
				break;
...
uint32_t mov_reader_objectid(mov_reader_t* mov, uint32_t track_id) {
	struct mov_track_t *track;
	struct mov_sample_entry_t *entry;

	for (int i = 0; i < mov->mov.track_count; i++) {
		track = &mov->mov.tracks[i];
		if (track->tkhd.track_ID == track_id) {
			break;
		}
	}
	
	entry = &track->stsd.entries[0];

	return entry->object_type_indication;
}

uint32_t mov_reader_gettrackcount(mov_reader_t *mov) {
	return mov->mov.track_count;
}

void mov_reader_video_entry_info(mov_reader_t *mov, uint8_t *obj_id, uint8_t *extra_data, int *extra_data_size) {
	struct mov_track_t *track;
	struct mov_sample_entry_t *entry;

	for (int i = 0; i < mov->mov.track_count; i++) {
		track = &mov->mov.tracks[i];
		for (int j = 0; j < track->stsd.entry_count && j < 1; j++) {
			entry = &track->stsd.entries[j];
			if (MOV_OBJECT_H264 == entry->object_type_indication ||
					MOV_OBJECT_HEVC == entry->object_type_indication) {
				memcpy(extra_data, entry->extra_data, entry->extra_data_size);
				*extra_data_size = entry->extra_data_size;
				*obj_id = entry->object_type_indication;
				return;
			}
		}
	}
	printf("has not video track.\n");
	return;
}

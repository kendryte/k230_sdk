...
struct mov_reader_trackinfo_t
{
	...
	void (*onvideo)(void* param, uint32_t track, uint32_t time_scale, uint8_t object, int width, int height, const void* extra, size_t bytes);
	void (*onaudio)(void* param, uint32_t track, uint32_t time_scale, uint8_t object, int channel_count, int bit_per_sample, int sample_rate, const void* extra, size_t bytes);
	void (*onsubtitle)(void* param, uint32_t track, uint8_t object, const void* extra, size_t bytes);
};

/// @return 0-OK, other-error
int mov_reader_getinfo(mov_reader_t* mov, uint32_t index, struct mov_reader_trackinfo_t *ontrack, void* param);

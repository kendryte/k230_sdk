...
{
    rt_err_t result;
    int rand_num = (rand() % 0x10000) + 1; // Eliminate the zero
...
static struct rt_thread _other_static_thread;
static rt_uint8_t _other_static_thread_stack[TEST_THREAD_STACK_SIZE];

static void _other_static_thread_entry(void *parameter)
{
    rt_thread_delay(rt_tick_from_millisecond(100));
    rt_sem_detach(&static_semaphore);
}

static void test_semaphore_detach_has_thread(void)
{
    rt_err_t result;
    ...
    result = rt_thread_init(&_other_static_thread,
                            "other_static_thread",
                            _other_static_thread_entry,
                            RT_NULL,
                            _other_static_thread_stack,
                            sizeof(_other_static_thread_stack),
    ...
    result = rt_thread_startup(&_other_static_thread);
    if (result != RT_EOK)
    {
        uassert_true(RT_FALSE);
        rt_sem_detach(&static_semaphore);
...
static void _other_dynamic_thread_entry(void *parameter)
{
    rt_thread_delay(rt_tick_from_millisecond(100));
    rt_sem_delete(dynamic_semaphore);
}

static void test_semaphore_delete_has_thread(void)
{
    rt_err_t result;
    rt_thread_t other_thread;

    dynamic_semaphore = rt_sem_create("dynamic_sem", 0, RT_IPC_FLAG_PRIO);
    if (dynamic_semaphore == RT_NULL)
    {
        uassert_true(RT_FALSE);
        return;
    }

    other_thread = rt_thread_create("other_thread",
                                    _other_dynamic_thread_entry,
                                    dynamic_semaphore,
                                    TEST_THREAD_STACK_SIZE,
                                    rt_thread_self()->current_priority,
                                    10);
    if (other_thread == RT_NULL)
    {
        uassert_true(RT_FALSE);
        rt_sem_delete(dynamic_semaphore);
        return;
    ...
    result = rt_thread_startup(other_thread);
    if (result != RT_EOK)
    {
        uassert_true(RT_FALSE);
        rt_sem_delete(dynamic_semaphore);
        rt_thread_delete(other_thread);
        return;
    }
...
static void testcase(void)
{
    ...
    UTEST_UNIT_RUN(test_semaphore_detach_has_thread);
#ifdef RT_USING_HEAP
    UTEST_UNIT_RUN(test_semaphore_delete_has_thread);
#endif
}

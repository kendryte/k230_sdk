...
static void test_mailbox_init(void)
{
    ...
    result = rt_mb_init(&test_static_mb, "mbt", &mb_pool[0], sizeof(mb_pool) / sizeof(void*), RT_IPC_FLAG_FIFO);
    if (result != RT_EOK)
    {
        uassert_false(1);
    }
    ...
    result = rt_mb_init(&test_static_mb, "mbt", &mb_pool[0], sizeof(mb_pool) / sizeof(void*), RT_IPC_FLAG_PRIO);
    if (result != RT_EOK)
    {
        uassert_false(1);
    }
...
static void test_mailbox_create(void)
{
    ...
    test_dynamic_mb = rt_mb_create("test_dynamic_mb", sizeof(mb_pool) / sizeof(void*), RT_IPC_FLAG_FIFO);
    ...
    test_dynamic_mb = rt_mb_create("test_dynamic_mb", sizeof(mb_pool) / sizeof(void*), RT_IPC_FLAG_PRIO);
    if (test_dynamic_mb == RT_NULL)
    {
        uassert_false(1);
    }
...
static void thread2_send_static_mb(void *arg)
{
    ...
    res = rt_mb_send_wait(&test_static_mb, (rt_ubase_t)&mb_send_str3, RT_WAITING_FOREVER);
    if (res != RT_EOK)
    {
        uassert_false(1);
    }
...
static void thread4_send_dynamic_mb(void *arg)
{
    ...
    res = rt_mb_send_wait(test_dynamic_mb, (rt_ubase_t)&mb_send_str3, RT_WAITING_FOREVER);
    if (res != RT_EOK)
    {
        uassert_false(1);
    }

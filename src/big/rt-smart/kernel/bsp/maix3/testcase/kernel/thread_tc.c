...
static void test_thread_yield(void)
{
    ...
    while (tid6_finish_flag != 1)rt_thread_delay(10);
    uassert_true(thread5_source >= 200);
...
#define ROUNDROBIN_THREAD_NUM   2
#define ROUNDROBIN_TEST_TIMES   5
static rt_thread_t rrtid[ROUNDROBIN_THREAD_NUM] = {0};
static rt_bool_t roundrobin_error = 0;

static void _scheduler_hook(struct rt_thread *from, struct rt_thread *to)
{
    if ((from == rrtid[0] || from == rrtid[1]) && (to == rrtid[0] || to == rrtid[1]))
    {
        if (from->remaining_tick != THREAD_TIMESLICE || to->remaining_tick != THREAD_TIMESLICE)
        {
            roundrobin_error = RT_TRUE;
        }
    }
}

static void thread_roundrobin_entry(void *parameter)
{
    rt_tick_t first_run_tick;

    first_run_tick = rt_tick_get() + ROUNDROBIN_TEST_TIMES * THREAD_TIMESLICE;

    while (!roundrobin_error)
    {
        if (rt_tick_get() - first_run_tick < (RT_TICK_MAX / 2))
        {
            uassert_true(1);
...
static void test_schedule_timeslice_roundrobin(void)
{
    rt_err_t result;
    int i;

    rt_scheduler_sethook(_scheduler_hook);

    roundrobin_error = RT_FALSE;
    for (i = 0; i < ROUNDROBIN_THREAD_NUM; i++)
    {
        rrtid[i] = rt_thread_create("threadroundrobin",
                                thread_roundrobin_entry,
                                RT_NULL,
                                THREAD_STACK_SIZE,
                                __current_thread->current_priority + 1,
                                THREAD_TIMESLICE);
        ...
        {
            LOG_E("rt_thread_create failed!");
            uassert_false(rrtid[i] == RT_NULL);
            return;
        }

        result = rt_thread_startup(rrtid[i]);
        if (result != RT_EOK)
        {
            LOG_E("rt_thread_startup failed!");
            uassert_false(1);
            rt_thread_delete(rrtid[i]);
            return;
        }
    }

    rt_thread_delay(ROUNDROBIN_THREAD_NUM * ROUNDROBIN_TEST_TIMES * THREAD_TIMESLICE + THREAD_TIMESLICE);

    uassert_true(!roundrobin_error);
}
...
static void testcase(void)
{
    ...
    UTEST_UNIT_RUN(test_schedule_timeslice_roundrobin);
}

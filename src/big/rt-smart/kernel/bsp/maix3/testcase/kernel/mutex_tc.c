...
static void static_mutex_take_entry(void *param)
{
    ...
    result = rt_mutex_take(mutex, 5);
    uassert_true(-RT_ETIMEOUT == result);
...
static void _static_mutex_detach_thread_entry(void *parameter)
{
    rt_err_t result;
    rt_mutex_t mtx;

    mtx = (rt_mutex_t)parameter;
...
static void test_static_mutex_detach(void)
{
    rt_err_t result;
    ...
    for (int i = 0; i < 5; i++)
    {
        rt_thread_t tid = rt_thread_create("mutex_th1",
                                        _static_mutex_detach_thread_entry,
                                        &static_mutex,
                                        TEST_THREAD_STACK_SIZE,
                                        10,
                                        10);
...
static void _dynamic_mutex_detach_thread_entry(void *parameter)
{
    rt_err_t result;
    rt_mutex_t mtx;
...
static void test_dynamic_mutex_detach(void)
{
    rt_err_t result;
    ...
    {
        rt_thread_t tid = rt_thread_create("mutex_th1",
                                        _dynamic_mutex_detach_thread_entry,
                                        dynamic_mutex,
                                        TEST_THREAD_STACK_SIZE,
...
static void testcase(void)
{
    ...
    UTEST_UNIT_RUN(test_static_mutex_detach);
    ...
    UTEST_UNIT_RUN(test_dynamic_mutex_detach);
#endif
}

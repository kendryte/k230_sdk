...
#define MSG_SIZE    (sizeof(rt_size_t))
#define MAX_MSGS    5

static struct rt_messagequeue static_mq;
static rt_uint8_t mq_buf[(MSG_SIZE + sizeof(void*)) * MAX_MSGS];

static struct rt_thread mq_send_thread;
static struct rt_thread mq_recv_thread;
static rt_uint8_t mq_send_stack[8192];
static rt_uint8_t mq_recv_stack[8192];
...
{
    rt_size_t send_buf[MAX_MSGS+1] = {0};
    rt_err_t ret = RT_EOK;
    ...
    send_buf[0] = 0x12;
    /* test mq size situation */
    ret = rt_mq_send_wait(testmq, &send_buf[0], 1, RT_WAITING_FOREVER);
    uassert_true(ret == RT_EOK);

    /* expect send failure because msg size out of MAX size. */
    ret = rt_mq_send_wait(testmq, &send_buf[0], MSG_SIZE + 1, RT_WAITING_FOREVER);
    uassert_true(ret != RT_EOK);
}
...
{
    rt_size_t recv_buf[MAX_MSGS+1] = {0};
    rt_err_t ret = RT_EOK;
    ...
    /* test mq size situation */
    ret = rt_mq_recv(testmq, &recv_buf[0], 1, RT_WAITING_FOREVER);
    uassert_true(ret == RT_EOK);
    uassert_true(recv_buf[0] == 0x12);

    /* expect recv failure */
    ret = rt_mq_recv(testmq, &recv_buf[0], sizeof(recv_buf[0]), RT_WAITING_FOREVER);
    uassert_true(ret != RT_EOK);
}
...
{
    rt_err_t ret = rt_event_detach(&finish_e);
    if(ret != RT_EOK)
        return -RT_ERROR;
    rt_thread_delay(100);
    return RT_EOK;

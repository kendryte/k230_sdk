...
ALIGN(RT_ALIGN_SIZE)
static char thread3_stack[TEST_THREAD_STACK_SIZE];
static struct rt_thread thread3;

ALIGN(RT_ALIGN_SIZE)
static char thread4_stack[TEST_THREAD_STACK_SIZE];
static struct rt_thread thread4;
#endif /* RT_USING_HEAP */

static rt_uint32_t recv_event_times1 = 0, recv_event_times2 = 0, recv_event_times3 = 0;
static rt_uint32_t static_event_recv_thread_finish = 0, static_event_send_thread_finish = 0;

ALIGN(RT_ALIGN_SIZE)
static char thread1_stack[TEST_THREAD_STACK_SIZE];
static struct rt_thread thread1;

ALIGN(RT_ALIGN_SIZE)
static char thread2_stack[TEST_THREAD_STACK_SIZE];
...
static void thread1_recv_static_event(void *param)
{
    ...
    /* wait send thread finish */
    while (static_event_send_thread_finish != 1)
    {
        rt_thread_mdelay(10);
    }

    recv_event_times3 = 0;
    /* expect: send 5times should recved 1 times */
    while (1)
    {
        e = 0;
        if (rt_event_recv(&static_event, EVENT_FLAG3,
                      RT_EVENT_FLAG_AND | RT_EVENT_FLAG_CLEAR,
...
static void thread2_send_static_event(void *param)
{
    ...
    rt_thread_delay(50);
    /* send 5 times, should recv only one times. */
    rt_event_send(&static_event, EVENT_FLAG3);
    rt_event_send(&static_event, EVENT_FLAG3);
    rt_event_send(&static_event, EVENT_FLAG3);
    rt_event_send(&static_event, EVENT_FLAG3);
...
static void test_static_event_send_recv(void)
{
    ...
    uassert_int_equal(recv_event_times3, 1);

    return;
}

static void test_event_flag_clear(void)
{
    rt_err_t result = RT_EOK;
    rt_uint32_t e;
    ...
    e = 0;
    rt_event_send(&static_event, EVENT_FLAG5);
    rt_event_recv(&static_event, EVENT_FLAG5, RT_EVENT_FLAG_AND, RT_WAITING_NO, &e);
    uassert_int_equal(e, EVENT_FLAG5);

    e = 0;
    rt_event_recv(&static_event, EVENT_FLAG5, RT_EVENT_FLAG_CLEAR | RT_EVENT_FLAG_AND, RT_WAITING_NO, &e);
    uassert_int_equal(e, EVENT_FLAG5);

    e = 0;
    rt_event_recv(&static_event, EVENT_FLAG5, RT_EVENT_FLAG_CLEAR | RT_EVENT_FLAG_AND, RT_WAITING_NO, &e);
    uassert_int_equal(e, 0);

    rt_event_send(&static_event, EVENT_FLAG3 | EVENT_FLAG5);
    ...
    e = 0;
    rt_event_recv(&static_event, EVENT_FLAG3 | EVENT_FLAG5, RT_EVENT_FLAG_CLEAR | RT_EVENT_FLAG_OR, RT_WAITING_NO, &e);
    uassert_int_equal(e, EVENT_FLAG3);
...
static void test_event_send_recv_allbits01(void)
{
    rt_err_t result = RT_EOK;
    rt_uint32_t e;
    ...
    result = rt_event_send(&static_event, 0);
    uassert_int_equal(result, -RT_ERROR);

    e = 0xFFFFFFFFU;
    result = rt_event_send(&static_event, e);
    uassert_int_equal(result, RT_EOK);

    e = 0;
    result = rt_event_recv(&static_event, 0xFFFFFFFFU, RT_EVENT_FLAG_AND | RT_EVENT_FLAG_CLEAR, RT_WAITING_NO, &e);
    uassert_int_equal(result, RT_EOK);
    uassert_int_equal(e, 0xFFFFFFFFU);
...
static void testcase(void)
{
    ...
    UTEST_UNIT_RUN(test_event_flag_clear);
    UTEST_UNIT_RUN(test_event_send_recv_allbits01);
#ifdef RT_USING_HEAP

                              ...
                              uint16_t size)
{
    ...
    usb_memcpy(data, g_asix_buf, ret - 8);
                               ...
                               uint16_t size)
{
    ...
    usb_memcpy(g_asix_buf, data, size);
...
static int usbh_asix_connect(struct usbh_hubport *hport, uint8_t intf)
{
    ...
    usb_memset(asix_class, 0, sizeof(struct usbh_asix));
...
find_class:
    ...
    while (1) {
        ...
        if (g_asix_rx_length % USB_GET_MAXPACKETSIZE(g_asix_class.bulkin->wMaxPacketSize)) {
            ...
            if (p != NULL) {
                usb_memcpy(p->payload, (uint8_t *)&g_asix_rx_buffer[4], len);
                g_asix_rx_length = 0;

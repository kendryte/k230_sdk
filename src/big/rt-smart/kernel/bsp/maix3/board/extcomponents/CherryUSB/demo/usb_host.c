...
find_class:
    ...
    usb_memset(cdc_speed_buffer, 0xAA, TEST_LEN);
...
#endif
    usb_memset(cdc_buffer, 0, 512);

    const uint8_t data1[10] = { 0x02, 0x00, 0x00, 0x00, 0x02, 0x02, 0x08, 0x14 };

    usb_memcpy(cdc_buffer, data1, 10);
...
int usb_msc_fatfs_test()
{
    ...
    for (uint32_t i = 0; i < 100; i++) {
        usb_memcpy(&read_write_buffer[i * 25], tmp_data, strlen(tmp_data));
    }
...
void usbh_cdc_ecm_run(struct usbh_cdc_ecm *cdc_ecm_class)
{
    ...
    netif->hwaddr_len = 6;
    usb_memcpy(netif->hwaddr, cdc_ecm_class->mac, 6);
...
void usbh_cdc_ncm_run(struct usbh_cdc_ncm *cdc_ncm_class)
{
    ...
    netif->hwaddr_len = 6;
    usb_memcpy(netif->hwaddr, cdc_ncm_class->mac, 6);
...
void usbh_rndis_run(struct usbh_rndis *rndis_class)
{
    ...
    usb_memset(&rndis_dev, 0, sizeof(struct eth_device));
    ...
    netif->hwaddr_len = 6;
    usb_memcpy(netif->hwaddr, rndis_class->mac, 6);
...
void usbh_asix_run(struct usbh_asix *asix_class)
{
    ...
    usb_memset(&asix_dev, 0, sizeof(struct eth_device));
    ...
    netif->hwaddr_len = 6;
    usb_memcpy(netif->hwaddr, asix_class->mac, 6);
...
void usbh_rtl8152_run(struct usbh_rtl8152 *rtl8152_class)
{
    ...
    usb_osal_thread_create("usbh_rtl8152_rx", 4096, CONFIG_USBHOST_PSC_PRIO + 1, usbh_rtl8152_rx_thread, rtl8152_dev.netif);
#else
    struct netif *netif = &g_rtl8152_netif;

    netif->hwaddr_len = 6;
    usb_memcpy(netif->hwaddr, rtl8152_class->mac, 6);

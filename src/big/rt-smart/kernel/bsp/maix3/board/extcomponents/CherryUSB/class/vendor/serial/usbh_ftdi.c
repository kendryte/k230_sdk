...
static struct usbh_ftdi *usbh_ftdi_class_alloc(void)
{
    ...
    for (devno = 0; devno < CONFIG_USBHOST_MAX_FTDI_CLASS; devno++) {
        if ((g_devinuse & (1 << devno)) == 0) {
            g_devinuse |= (1 << devno);
            usb_memset(&g_ftdi_class[devno], 0, sizeof(struct usbh_ftdi));
...
static void usbh_ftdi_class_free(struct usbh_ftdi *ftdi_class)
{
    ...
    usb_memset(ftdi_class, 0, sizeof(struct usbh_ftdi));
}
...
static int usbh_ftdi_read_modem_status(struct usbh_ftdi *ftdi_class)
{
    ...
    usb_memcpy(ftdi_class->modem_status, g_ftdi_buf, 2);
    return ret;
}

int usbh_ftdi_set_line_coding(struct usbh_ftdi *ftdi_class, struct cdc_line_coding *line_coding)
{
    usb_memcpy((uint8_t *)&ftdi_class->line_coding, line_coding, sizeof(struct cdc_line_coding));
...
{
    usb_memcpy(line_coding, (uint8_t *)&ftdi_class->line_coding, sizeof(struct cdc_line_coding));
    return 0;
}
...
static int usbh_ftdi_connect(struct usbh_hubport *hport, uint8_t intf)
{
    ...
    usb_memset(g_ftdi_buf, 'a', sizeof(g_ftdi_buf));

...
def DefineSet(defines):
    set = []
    for item in defines:
        if type(item) == type('str'):
            set.append(item)
        elif type(item) == type(('str', )):
            set.extend(DefineSet(item))
...
def GenerateCFiles(env):
    ...
    if vsc_file:
        cc = os.path.join(env['ArchConfig'].EXEC_PATH, env['ArchConfig'].CC)
        cc = os.path.abspath(cc).replace('\\', '/')

        config_obj = {}
        config_obj['name'] = 'Win32'
        config_obj['defines'] = DefineSet(env['CPPDEFINES'])
        config_obj['intelliSenseMode'] = 'gcc-arm'

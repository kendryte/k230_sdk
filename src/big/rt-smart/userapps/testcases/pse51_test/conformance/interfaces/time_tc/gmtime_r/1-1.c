...
int main(void)
{
	...
	(void)time(&the_time);
	tm_ptr = gmtime_r(&the_time, &tm_buf);
	printf("Raw time is %ld \n", the_time);
	printf("gmtime gives:\n");

	/* Checking the seconds */
	if ((tm_buf.tm_sec >= 0) && (tm_buf.tm_sec < 60)) {
		printf("sec %02d\n", tm_buf.tm_sec);
	} else {
	...
	if ((tm_buf.tm_min >= 0) && (tm_buf.tm_min <= 59)) {
		printf("min %02d\n", tm_buf.tm_min);
	} else {
	...
	/* Checking the Hour */
	if ((tm_buf.tm_hour >= 0) && (tm_buf.tm_hour <= 23)) {
		printf("hour %02d\n", tm_buf.tm_hour);
	} else {
	...
	if ((tm_buf.tm_mday >= 1) && (tm_buf.tm_mday <= 31)) {
		printf("mday %02d\n", tm_buf.tm_mday);
	} else {
	...
	/* Checking the Year */
	total_years = (tm_buf.tm_year + 1900);
	if (total_years >= 1900) {
		printf("year %d\n", total_years);
	} else {
		printf("year %d\n", total_years);
	...
	if ((tm_buf.tm_yday >= 0) && (tm_buf.tm_yday <= 365)) {
		printf("yday %02d\n", tm_buf.tm_yday);
	} else {
	...
	/* Checking the DTS */
	if ((tm_buf.tm_isdst >= -1) && (tm_buf.tm_isdst <= 1)) {
		printf("isdst %02d\n", tm_buf.tm_isdst);
	} else {

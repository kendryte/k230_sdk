...
from utils.config import conf, load_config
...
class Session(object):
    ...
    def build_session_query(query, session_id):
        ...
        if len(session) == 0:
            system_prompt = conf().get("character_planning", "")
            with open(conf().get("action command", ""), encoding='utf-8') as file:
                messages = [{"role": "system", "content": 'action command:' + f'{json.load(file)}'}]

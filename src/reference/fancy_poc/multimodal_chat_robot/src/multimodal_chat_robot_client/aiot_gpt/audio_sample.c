...
static void _test_ai_i2s_in_data(const char *filename, int dev_num, int channel, k_audio_bit_width bit_width, int sample_rate, int nTotal_Sec)
{
...
#if ENABLE_SAVE_PCM
    audio_save_init(filename, sample_rate, 2, bit_width, SAVE_PCM_SECOND);
#endif
...
k_s32 audio_sample_get_ai_i2s_data(const char *filename, k_audio_bit_width bit_width, k_u32 sample_rate, k_i2s_work_mode i2s_work_mode, k_bool enable_audio3a)
{
    ...
    _test_ai_i2s_in_data(filename, 0, 0, bit_width, sample_rate, -1);
...
k_s32 audio_sample_send_ao_data(const char *filename, int nDev, int nChannel, int samplerate, k_audio_bit_width bit_width, k_i2s_work_mode i2s_work_mode)
{
    ...
    {
        // 获取音频数据
        // get_pcm_data_from_file(pDataBuf, audio_frame.len / 4, &eof_flag);
        if (-1 == get_pcm_data_from_file(pDataBuf, audio_frame.len / 4))
        {
            sleep(1);
            break;
        }

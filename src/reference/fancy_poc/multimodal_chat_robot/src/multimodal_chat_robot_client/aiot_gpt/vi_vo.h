...
#define SENSOR_WIDTH  (1280 / 2)    // isp宽度，ai输入，横屏，水平方向为x轴
#define SENSOR_HEIGHT (720 / 2)    // isp高度，ai输入，横屏
...
static k_s32 vo_layer_vdss_bind_vo_config(void)
{
    ...
    attr.bg_color = 0x808000; // 0x808000;0xffffff
...
int vivcap_start()
{
    ...
    chn_attr.out_win.h_start = 0;
    chn_attr.out_win.v_start = 0;
    // chn_attr.out_win.width = HEIGHT;
    // chn_attr.out_win.height = WIDTH;
    ...
    // chn_attr.dw_enable = K_FALSE;
    chn_attr.chn_enable = K_TRUE;
    chn_attr.pix_format = PIXEL_FORMAT_BGR_888_PLANAR; // rgb
    chn_attr.buffer_num = VICAP_MAX_FRAME_COUNT;       // at least 3 buffers for isp

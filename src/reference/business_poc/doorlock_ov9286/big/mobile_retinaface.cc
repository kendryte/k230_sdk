...
void MobileRetinaface::preprocess(uintptr_t vaddr, uintptr_t paddr)
{
...
#if ENABLE_DEBUG
    {
        ...
        dump_to_bin("retinaface_ai2d.bin", reinterpret_cast<void *>(buf.data()), buf.size());
    }
#endif
}
...
#if ENABLE_DEBUG
    std::cout << "box_num = " << result_.boxes.size() << ", landmark_num = " << result_.landmarks.size() << std::endl;
#endif

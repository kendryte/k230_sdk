...
k_vb_blk_handle get_vo_insert_frame(k_video_frame_info *vf_info, void **pic_vaddr) {
    k_u64 phys_addr = 0;
    k_u32 *virt_addr;
...
static void sample_vicap_bind(k_mpp_chn vicap_mpp_chn, k_mpp_chn vo_mpp_chn) {
    k_s32 ret;
...
static void sample_vicap_unbind(k_mpp_chn vicap_mpp_chn, k_mpp_chn vo_mpp_chn) {
    k_s32 ret;
...
int vivcap_start() {
    ...
    media_attr.media_config.vb_config.comm_pool[0].blk_size = AUDIO_SAMPLE_RATE * 2 * 4 / AUDIO_PERSEC_DIV_NUM;
    ...
    media_attr.media_config.vb_config.comm_pool[1].blk_size = AUDIO_SAMPLE_RATE * 2 * 4 / AUDIO_PERSEC_DIV_NUM * 2;
    media_attr.media_config.vb_config.comm_pool[1].mode = VB_REMAP_MODE_NOCACHE;
    //vb for video
    k_u64 pic_size = VIDEO_WIDTH * VIDEO_HEIGHT * 3 / 2;
    k_u64 stream_size = VIDEO_WIDTH * VIDEO_HEIGHT / 2;
    ...
    media_attr.media_config.vb_config.comm_pool[4].blk_size = VICAP_ALIGN_UP((ISP_CHN0_WIDTH * ISP_CHN0_HEIGHT * 3 / 2), VICAP_ALIGN_1K);
    ...
    media_attr.media_config.vb_config.comm_pool[5].blk_size = VICAP_ALIGN_UP((SENSOR_HEIGHT * SENSOR_WIDTH * 3 ), VICAP_ALIGN_1K);

    memset(&sensor_info, 0, sizeof(sensor_info));
    kd_mpi_vicap_get_sensor_info(sensor_type, &sensor_info);

    media_attr.media_config.vb_config.comm_pool[6].blk_cnt = 6;
    media_attr.media_config.vb_config.comm_pool[6].blk_size = VI_ALIGN_UP(sensor_info.width * sensor_info.height * 3 / 2, VICAP_ALIGN_1K);
    ...
    chn_attr.buffer_size = VI_ALIGN_UP(sensor_info.width * sensor_info.height * 3 / 2, VICAP_ALIGN_1K);// media_attr.media_config.vb_config.comm_pool[4].blk_size;
    ...
    sample_vicap_bind(vicap_mpp_chn, vo_mpp_chn);
    ...
    chn_attr.buffer_num = 6;//at least 3 buffers for isp
    chn_attr.buffer_size = media_attr.media_config.vb_config.comm_pool[5].blk_size;
    ...
    chn_attr.out_win.h_start = 0;
    chn_attr.out_win.v_start = 0;
    chn_attr.out_win.width = VIDEO_WIDTH ;
    chn_attr.out_win.height = VIDEO_HEIGHT;
    chn_attr.crop_win.h_start = 0;
    chn_attr.crop_win.v_start = 0;
    chn_attr.crop_win.width = VIDEO_WIDTH;
    chn_attr.crop_win.height = VIDEO_HEIGHT;
    chn_attr.scale_win = chn_attr.out_win;
    chn_attr.crop_enable = K_TRUE;
    chn_attr.scale_enable = K_FALSE;
    chn_attr.chn_enable = K_TRUE;
    ...
    chn_attr.buffer_num = 6;//at least 3 buffers for isp
    chn_attr.buffer_size = media_attr.media_config.vb_config.comm_pool[2].blk_size;
    ret = kd_mpi_vicap_set_chn_attr(vicap_dev, VICAP_CHN_ID_2, chn_attr);

    k_venc_chn_attr attr;
    memset(&attr, 0, sizeof(attr));
    attr.venc_attr.pic_width = VIDEO_WIDTH;
    attr.venc_attr.pic_height = VIDEO_HEIGHT;
    attr.venc_attr.stream_buf_size = ((stream_size + 0xfff) & ~0xfff);
...
int vivcap_stop() {
    ...
    venc_mpp_chn.dev_id = 0;
    venc_mpp_chn.chn_id = 0;
    sample_vicap_unbind(vi_mpp_chn, venc_mpp_chn);

    int ret = kd_mpi_venc_destroy_chn(0);
    if (ret) {
        printf("sample_vicap, kd_mpi_venc_destroy_chn failed.\n");
        return ret;
    }
    ...
    sample_vicap_unbind(vicap_mpp_chn, vo_mpp_chn);

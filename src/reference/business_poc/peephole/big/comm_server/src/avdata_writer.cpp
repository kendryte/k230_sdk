  ...
  public:
    ...
    int Init(uint64_t &data_fifo_phy_addr, uint64_t &data_phy_addr, uint64_t &mem_size, size_t mem_buf_size) {
        ...
        if (shm_.Init(data_phy_addr, virt_addr_, shm_size_, mem_buf_size) < 0) {
            std::cout << "AVDataWriter::Impl::Init() shm_init failed" << std::endl;
            return -1;
        }
        mem_size = shm_size_;
        //
    ...
    int Write(const AVEncFrameData &data, IInterruptCallback *callback) {
        ...
        shm_.FlushCache((uint8_t*)virt_addr_ + offset, block.size);
        ...
        // std::cout << "AVDataWriter::Impl::Write() called, avail_len_ = " << avail_len_ << ", write_idx = " << offset << ", size = " << data.size << std::endl;
        return 0;
    }
    ...
    virtual void release(void *opaque) override {
        ...
        // std::cout << "AVDataWriter::Impl::release() called, avail_len_ = " << avail_len_ << ", read_idx = " << read_idx_ << ", size = " << block->size << std::endl;
        read_idx_ += size;
...
int AVDataWriter::Init(uint64_t &data_fifo_phy_addr, uint64_t &data_phy_addr, uint64_t &mem_size, size_t mem_buf_size) {

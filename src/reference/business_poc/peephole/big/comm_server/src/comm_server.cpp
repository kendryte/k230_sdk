...
class UserCommServer::Impl {
  ...
  private:
    ...
    k_s32 ipcmsg_id_{-1};
    std::atomic<int> ipc_msg_id_atomic_{-1};
    std::thread thread_;
    std::mutex mutex_;
...
int UserCommServer::Impl::Init(const std::string &service_name, IServerCallback *callback, int port) {
    ...
    ipc_msg_id_atomic_.store(-1);
    thread_ = std::thread([this, port]() {
        std::cout << "UserCommServer::Impl::Init() -- ipc thread ...1" << std::endl;
        ...
        k_s32 ret = kd_ipcmsg_add_service(service_name_.c_str(), &stConnectAttr);
        if (ret != K_SUCCESS) {
            return -1;
        }
        std::cout << "UserCommServer::Impl::Init() -- ipc thread ...2" << std::endl;
        ...
        std::cout << "UserCommServer::Impl::Init() -- ipc thread...3" << std::endl;

        ipc_msg_id_atomic_.store(ipcmsg_id_);
        ...
        std::cout << "UserCommServer::Impl::Init() -- ipc thread...EXIT" << std::endl;
    });
    initialized_ = true;
    return 0;
}
...
void UserCommServer::Impl::DeInit() {
    ...
    if (ipc_msg_id_atomic_.load() != -1) {
        kd_ipcmsg_disconnect(ipc_msg_id_atomic_.load());
        ipc_msg_id_atomic_.store(-1);
        if (thread_.joinable()) {
            thread_.join();
        }

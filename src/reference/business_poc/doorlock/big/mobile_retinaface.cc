...
void MobileRetinaface::postprocess()
{
    ...
    pred_box.reserve(16);
    result_.landmarks.reserve(16);
    result_.landmarks.clear();
    decode(pred_box, result_.landmarks);
    ...
    for (size_t i = 0; i < pred_box.size(); i++)
    {
        ...
        // std::cout << "face_location0: " << box.x1 << ", " << box.y1 << ", " << box.x2 << ", " << box.y2 << std::endl;
    }
    // std::cout << "box_num = " << result_.boxes.size() << std::endl;
}

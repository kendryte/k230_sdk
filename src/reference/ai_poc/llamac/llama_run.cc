...
void malloc_run_state(RunState* s, Config* p) {
    ...
    s->k = reinterpret_cast<float *>(calloc(p->dim, sizeof(float)));
    s->v = reinterpret_cast<float *>(calloc(p->dim, sizeof(float)));
    ...
    s->key_cache = reinterpret_cast<float *>(calloc(p->n_layers * p->seq_len * p->dim, sizeof(float)));
    s->value_cache = reinterpret_cast<float *>(calloc(p->n_layers * p->seq_len * p->dim, sizeof(float)));
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2) {
        printf("Usage: %s <checkpoint_file> [temperature] [steps] [word]\n", argv[0]);
        return 1;
    }
    if (argc >= 2) {
        checkpoint = argv[1];
    ...
    long file_size;
    {
        ...
        // std::cout << "checkpoint_init_weights" << std::endl;
    }
    // right now we cannot run for more than config.seq_len steps  
    // model.bin 的 seq_len 为 256  （最大续写长度为256）
    ...
    std::map<std::string,int> vocab_map;
    {
        ...
        for (int i = 0; i < config.vocab_size; i++) {
            ...
            vocab_map.insert(pair<string, int>(tmp, i));

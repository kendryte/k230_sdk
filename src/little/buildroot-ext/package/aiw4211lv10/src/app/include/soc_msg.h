...
#ifndef __SOC_MSG_H__
#define __SOC_MSG_H__
...
typedef enum
{
    ...
    SOCCHN_CMD_SET_IP,
    SOCCHN_CMD_GET_IP,
    SOCCHN_CMD_SET_SLEEP,
    SOCCHN_CMD_SET_CONNECT,
    SOCCHN_CMD_SET_KEEPALIVE,
    SOCCHN_CMD_SET_CONFIG,
    SOCCHN_CMD_END,
} socchn_cmd_e;
...
typedef struct
{
    unsigned int level;     //wifi休眠等级
#define WIFI_SLEEP_EXIT         0
#define WIFI_SLEEP_LIGHT        1
#define WIFI_SLEEP_DEEP         2
#define WIFI_SLEEP_ULTRA        3
    unsigned int period;    //预期休眠时间
    union 
    {
        /* 
...
typedef enum {
    ...
    EXT_WIFI_SECURITY_WPA,                   /* WPA. CNcomment: 认证类型:WPA.CNend */
    EXT_WIFI_SECURITY_WPA2,                  /* WPA2. CNcomment: 认证类型:WPA2.CNend */
...
typedef struct
{
    unsigned char  svrip[16];//server端ip地址
    unsigned short svrport; //server端端口号
    unsigned short tcp_keepalive_time;      //TCP层keepalive心跳包常规发送周期
    unsigned short tcp_keepalive_intvl;     //TCP层keepalive心跳包无回应之后新的发送周期
    unsigned short tcp_keepalive_probes;    //TCP层keepalive心跳包无回应计数超过该阈值之后，判断TCP连接异常，断开重连
    unsigned short heartbeat_intvl;     //应用层心跳包发送周期，second
    unsigned short heartbeat_probes;    //应用层心跳包无回应计数超过该阈值之后，判断socket连接异常，断开重连
} wifi_keepalive_t;

typedef struct
{
    unsigned int magic; //魔数
#define MAGIC_NUM   (0xA5F0B4C7)
    unsigned int cmd;   //指令
    unsigned int length;//消息包总长
    unsigned int crc;   //消息包crc32校验值
} socchn_msg_hdr_t;

typedef struct
{
    socchn_msg_hdr_t header;
    union {
        wifi_mac_t mac; //mac地址
        wifi_ip_t  ip;  //ip地址
        wifi_sleep_t sleep; //wifi休眠参数
        wifi_connect_t conn;   //wifi连接ap参数
        wifi_keepalive_t keepalive; //wifi链路保活参数
        unsigned char dat[112];
    };
...
#define MSG_TYPE_HRBTINIT    ('H'<<24|'R'<<16|'I'<<8|'N')
#define MSG_TYPE_HRBT        ('H'<<24|'R'<<16|'B'<<8|'T')
#define MSG_TYPE_WAKEUP      ('W'<<24|'K'<<16|'U'<<8|'P')
    union {
        unsigned int heartbeat_intvl;   //告知服务端wifi应用层心跳周期，当type == MSG_TYPE_HRBTINIT时使用，其他type不使用
        unsigned int length;            //data数组长度，用于扩展消息体，暂不使用
    };
    unsigned char data[0];              //data数组，用于扩展消息体，暂不使用
} remote_msg_t;
...
static inline unsigned int crc_32 ( unsigned int val, const void *ss, int len )
{

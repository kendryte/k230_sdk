...
static int drm_setup(unsigned int fourcc)
{
	...
	info("drm: Found plane_id: %u connector_id: %u crtc_id: %u",
		drm_dev.plane_id, drm_dev.conn_id, drm_dev.crtc_id);

	info("drm: %ux%u (%umm X%u mm) pixel format %c%c%c%c",
...
int drm_display(unsigned index) {
	if (drm_dev.req)
		drm_wait_vsync();
	return drm_dmabuf_set_plane(&drm_dev.drm_bufs[index]);
}
...
void drm_exit(void)
{
	for (unsigned i = 0; i < 2; i++) {
		struct drm_mode_destroy_dumb dd = {
			.handle = drm_dev.drm_bufs[i].handle
		};
		drmModeRmFB(drm_dev.fd, drm_dev.drm_bufs[i].fb_handle);
		drmIoctl(drm_dev.fd, DRM_IOCTL_MODE_DESTROY_DUMB, &dd);
	}

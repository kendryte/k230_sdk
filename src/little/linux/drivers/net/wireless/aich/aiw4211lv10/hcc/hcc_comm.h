...
#define hcc_get_event_payload(pst_event)    ((td_u8*)((pst_event)->auc_event_data))
...
typedef struct {
    td_u8           type;         /* 事件类型 */
    td_u8           sub_type;     /* 事件子类型 */
    td_u8           chip_id;      /* 芯片ID */
    td_u8           device_id;    /* 设备ID */
    td_u8           vap_id;       /* VAP ID */
    td_u8           resever;      /* 保留 */
    td_u16          us_length;    /* payload length */
...
/* 4bytes */
struct hcc_header {
    td_u16      sub_type : 4;        /* sub type to hcc type,refer to hcc_action_type */
    td_u16      main_type : 3;       /* main type to hcc type,refer to hcc_action_type */
    td_u16      pad_hdr : 6;         /* pad_hdr only need 6 bits, pad_hdr used to algin hcc tcm hdr(64B) */
    td_u16      pad_align : 2;       /* the pad to align the payload addr */
    td_u16      more : 1;            /* for hcc aggregation */
    td_u16      seq : 4;             /* seq num for debug */
    td_u16      pay_len : 12;        /* the payload length, did't contain the extend hcc hdr area */
};
#pragma pack(pop)

typedef struct hcc_header hcc_header_stru;
#define FRW_EVENT_TYPE_ENUM_UINT8 td_u8
...
#define HCC_EXTEND_TOTAL_SIZE  (WLAN_MAX_MAC_HDR_LEN + EXT_MAX_DEV_CB_LEN + (td_u32)sizeof(frw_hcc_extend_hdr_stru))
...
#define hcc_get_test_cmd_data(base) (((td_u8*)(base)) + sizeof(hcc_test_cmd_stru))

typedef struct _hsdio_trans_test_info_ {
    td_u32 actual_tx_pkts;
    td_u8  resv[4];                 /* 4 byte保留字段 */
    td_u64 total_h2d_trans_bytes;   /* total bytes trans by from host to device */
    td_u64 total_d2h_trans_bytes;   /* total bytes trans by from device to host */
} hsdio_trans_test_info;

typedef struct _hsdio_trans_test_rx_info_ {
    td_u32 total_trans_pkts;
    td_u32 pkt_len;
    td_u8  pkt_value;
    td_u8  resv[3]; /* 3 byte保留字段，word对齐 */
...
typedef td_u8 hcc_netbuf_queue_type;
...
typedef td_void (*hcc_free_uc_buf)(td_void* data);
...
typedef hcc_unc_struc* (*hcc_alloc_uc_buf)(td_s32 len, hcc_stru_type type);
typedef td_s32 (*hcc_rx)(td_void *data);
typedef td_u32 (*wlan_pm_wakeup_dev)(td_void *data);
...
typedef struct {
    ...
    td_u8                   flow_flag;          /* 协议栈流程标记位 */
    td_u8                   resv[3];            /* 3: 四字节对齐 */
}hcc_data_queue;
...
static inline td_void hcc_hdr_param_init(hcc_transfer_param *param, td_u32 main_type, td_u32 sub_type,
    td_u8 extend_len, td_u32 fc_type, td_u32 queue_id)
{

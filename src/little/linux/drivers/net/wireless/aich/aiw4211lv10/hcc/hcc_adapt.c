...
static td_char g_test_buf[MAX_TEST_PACKAGE_LEN] = {0};
#endif
static atomic_t g_tx_num = {0};
static td_void hcc_adapt_mem_free(td_void *data);
static td_void hcc_rx_test_speed(hcc_header_stru *hcc_hdr, td_char* buf, int len);

hcc_unc_struc* hcc_structure_conversion(const td_void* priv, hcc_stru_type type)
{
    oal_netbuf_stru *netbuf = TD_NULL;
    hcc_normal_struc* p_normal = TD_NULL;
...
static td_void hcc_adapt_netbuf_rx_data_process(oal_netbuf_stru *netbuf, hcc_header_stru *hcc_hdr)
{
    ...
    switch (hcc_hdr->main_type) {
        case HCC_TYPE_DATA:
            if (wal_rx_data_proc(netbuf) != EXT_SUCCESS) {
        ...
        case HCC_TYPE_TEST_XFER:
            ...
            break;
        case HCC_TYPE_MSG:
            if (oal_send_user_msg(oal_netbuf_data(netbuf), oal_netbuf_len(netbuf)) != EXT_SUCCESS) {
...
static td_u32 hcc_host_check_hdr(hcc_header_stru *hcc_hdr)
{
...
/*****************************************************************************
 ...
 返 回 值  : EXT_SUCCESS 消息处理成功， EXT_FAIL 消息处理失败，消息头异常
*****************************************************************************/
static td_u32 hcc_adapt_rx_data_process(hcc_unc_struc *unc_buf)
{
    oal_netbuf_stru *netbuf = TD_NULL;
    hcc_header_stru *hcc_hdr = (hcc_header_stru *)oal_unc_data(unc_buf);
    if (hcc_host_check_hdr(hcc_hdr) != TD_TRUE) {
...
td_u32 hcc_tx_hcc_hdr_init(hcc_unc_struc *unc_buf, const hcc_transfer_param *param)
{
...
static td_u32 hcc_host_tx(hcc_handler_stru *hcc_handler, oal_netbuf_stru *netbuf, const hcc_transfer_param *param)
{
    td_u32 ret;
    hcc_trans_queue_stru *hcc_queue = TD_NULL;
    ...
    if ((hcc_handler->hcc_bus_ops != TD_NULL) &&
        (hcc_handler->hcc_bus_ops->stop_tcpip_tx_queue != TD_NULL)) {
...
{
    td_s32 ret;
    td_u32 len_algin, tail_room_len;
    td_u32 len = oal_netbuf_len(netbuf);
    ...
    if (oal_unlikely(tail_room_len > oal_netbuf_tailroom(netbuf))) {
        /* tailroom not enough */
        ret = oal_netbuf_expand_head(netbuf, 0, (td_s32)tail_room_len, GFP_KERNEL);
        if (oal_warn_on(ret != EXT_SUCCESS)) {
            oal_free(unc_buf);
            return TD_NULL;
        }
    }
    ...
    unc_buf->buf = oal_netbuf_data(netbuf);
    unc_buf->length = oal_netbuf_len(netbuf);
    unc_buf->priv = (td_void *)netbuf;
    unc_buf->priv_type = NETBUF_STRU_TYPE;
    unc_buf->free = hcc_adapt_mem_free;
    return unc_buf;
}

static hcc_unc_struc* hcc_adapt_netbuf_alloc(td_s32 len)
{
    oal_netbuf_stru *netbuf;
    netbuf = __netdev_alloc_skb(TD_NULL, len, GFP_KERNEL);
    if (netbuf == TD_NULL) {
...
static hcc_unc_struc* hcc_adapt_nomal_alloc(td_s32 len)
{
    hcc_normal_struc p_normal;

    p_normal.buf = oal_memalloc(len);
    if (p_normal.buf  == TD_NULL) {
        return TD_NULL;
    }
    p_normal.length = len;
    /* 重新设置unc配置 */
...
/*****************************************************************************
 ...
 输入参数  : queue_id 队列id
             hcc_queue队列头指针
             unc_buf 通用数据结构指针
 返 回 值  : EXT_SUCCESS 可以丢掉此帧，EXT_FAIL此帧不能丢掉
*****************************************************************************/
static td_u32 hcc_adapt_check_discard_frame(td_u32 queue_id,
...
*****************************************************************************/
td_void hcc_adapt_stop_tcpip_tx_queue(hcc_trans_queue_stru *hcc_queue)
{
    if ((hcc_list_len(&hcc_queue->queue_info) > MAX_CNT_IN_QUEUE) &&
        (hcc_queue->queue_info.flow_flag == TD_FALSE)) {
        ...
        hcc_queue->queue_info.flow_flag = TD_TRUE;
        oal_netif_stop_queue(netdev);
    }
}
...
*****************************************************************************/
td_void hcc_adapt_awake_tcpip_tx_queue(hcc_trans_queue_stru *hcc_queue)
{
    if ((hcc_list_len(&hcc_queue->queue_info) < AWAKE_CNT_IN_QUEUE) && (hcc_queue->queue_info.flow_flag == TD_TRUE)) {
        oal_net_device_stru *netdev = oal_get_netdev_by_name("wlan0");
        if (netdev == TD_NULL) {
...
/*****************************************************************************
 ...
 返 回 值  : 返回通用数据结构指针
*****************************************************************************/
static hcc_unc_struc* hcc_adapt_alloc_unc_buf(td_s32 len, hcc_stru_type type)
{
    switch (type) {
        case NETBUF_STRU_TYPE:
...
/*****************************************************************************
 ...
 返 回 值  : EXT_SUCCESS 内存充足，继续发送数据， EXT_FAIL device内存不足，停止发
             送数据
*****************************************************************************/
static td_u32 hcc_adapt_tx_flow_ctrl_handle(td_u16 flow_type, td_u8 dev_mem_cnt)
{
    if (flow_type == HCC_FLOWCTRL_CREDIT) {
        if (dev_mem_cnt <= EXT_PRI_MEM_LOW_LEVEL) {
            return EXT_FAIL;
        }
...
static td_void hcc_rx_test_speed(hcc_header_stru *hcc_hdr, td_char* buf, int len)
{
    if (len < sizeof(td_s32)) {
        return;
    }

    if (hcc_hdr->sub_type == HCC_TEST_READ_START) {
        oam_error_log1("Read start.package:%d", *((td_u32*)buf));
    } else if (hcc_hdr->sub_type == HCC_TEST_READ_TRANFER) {
        oam_error_log1("Package Num:%d", *((td_u32*)buf));
    } else if (hcc_hdr->sub_type == HCC_TEST_READ_OVER) {
        oam_error_log1("Read over.package:%d", *((td_u32*)buf));
    }
}

#ifdef _PRE_SOCCHANNEL_DEBUG
static void hcc_test_write(char* buf, int len, hcc_type_enum type, td_u32 sub_type)
{
    oal_netbuf_stru *netbuf = TD_NULL;

    netbuf = oal_netbuf_alloc(len + HCC_HDR_TOTAL_LEN + BUS_H2D_SCATT_BUFFLEN_ALIGN, 0, 4); /* 4: 4字节对齐 */
    if (netbuf == TD_NULL) {
    ...
    td_u32 ret = hcc_tx_data_adapt(netbuf, type, sub_type);
    if (ret != EXT_SUCCESS) {
...
/*****************************************************************************
 ...
 返 回 值  : 无
*****************************************************************************/
td_void hcc_test_channel_speed(td_void)
{
    ...
    speed = len / mlen;
    printk("test write over. speed:%lld Mbps\n", (speed / (end_time.tv_sec - start_time.tv_sec)) * 8); /* 8: Byte,bit转换 */
}
#endif

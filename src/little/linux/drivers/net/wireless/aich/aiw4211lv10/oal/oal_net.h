...
/* inline 函数定义 */
static inline oal_sock_stru* oal_netlink_kernel_create(td_void (*input)(oal_netbuf_stru *pst_netbuf))
{
...
static inline td_void oal_ether_setup(oal_net_device_stru *net_device)
{
    if (net_device == TD_NULL) {
        return;
    }
...
static inline oal_net_device_stru* oal_get_netdev_by_name(const td_char* pc_name)
{
    return dev_get_by_name(&init_net, pc_name);
}

static inline td_void oal_net_unregister_netdev(oal_net_device_stru* netdev)
{
    if (netdev == TD_NULL) {
        return;
    }
...
static inline td_void oal_netif_stop_queue(oal_net_device_stru* netdev)
{
    if (netdev == TD_NULL) {
        return;
    }
...
static inline td_void oal_netif_wake_queue(oal_net_device_stru* netdev)
{
    if (netdev == TD_NULL) {
        return;
    }

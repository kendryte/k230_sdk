...
void k230_dw_spi_set_cs(struct spi_device *spi, bool enable)
{
...
EXPORT_SYMBOL_GPL(k230_dw_spi_set_cs);
...
/* for st7735s test */
static void k230_dw_writer(struct dw_spi *dws)
{
	u32 txw = 0;
	u32 tx_room = 0;
	u32 len = 0;
	...
	while (dw_readl(dws, DW_SPI_TXFLR));
...
int k230_dw_spi_check_status(struct dw_spi *dws, bool raw)
{
	u32 irq_status;
	int ret = 0;
...
EXPORT_SYMBOL_GPL(k230_dw_spi_check_status);

static irqreturn_t dw_spi_transfer_handler(struct dw_spi *dws)
{
	u16 irq_status = dw_readl(dws, DW_SPI_ISR);

	if (k230_dw_spi_check_status(dws, false)) {
		spi_finalize_current_transfer(dws->master);
		return IRQ_HANDLED;
	}
...
void k230_dw_spi_update_config(struct dw_spi *dws, struct spi_device *spi,
...
EXPORT_SYMBOL_GPL(k230_dw_spi_update_config);
				...
				struct spi_transfer *transfer)
{
	...
	do {
		...
		ret = k230_dw_spi_check_status(dws, true);
		if (ret)
			return ret;
	} while (dws->rx_len);
		...
		struct spi_device *spi, struct spi_transfer *transfer)
{
	...
	k230_dw_spi_update_config(dws, spi, &cfg);
...
static int dw_spi_exec_mem_op(struct spi_mem *mem, const struct spi_mem_op *op)
{
	...
	k230_dw_spi_update_config(dws, mem->spi, &cfg);
...
static int dw_spi_try_enh_mem_op(struct spi_mem *mem, const struct spi_mem_op *op)
{
		...
		k230_dw_spi_set_cs(mem->spi, false);
		spi_enable_chip(dws, 1);
		if(cpu_wr){
...
int k230_dw_spi_add_host(struct device *dev, struct dw_spi *dws)
{
	...
	if (dws->set_cs)
		...
		master->set_cs = k230_dw_spi_set_cs;
...
EXPORT_SYMBOL_GPL(k230_dw_spi_add_host);

void k230_dw_spi_remove_host(struct dw_spi *dws)
{
...
EXPORT_SYMBOL_GPL(k230_dw_spi_remove_host);

int k230_dw_spi_suspend_host(struct dw_spi *dws)
{
	int ret;
...
EXPORT_SYMBOL_GPL(k230_dw_spi_suspend_host);

int k230_dw_spi_resume_host(struct dw_spi *dws)
{
	spi_hw_init(&dws->master->dev, dws);
	return spi_controller_resume(dws->master);
}
EXPORT_SYMBOL_GPL(k230_dw_spi_resume_host);

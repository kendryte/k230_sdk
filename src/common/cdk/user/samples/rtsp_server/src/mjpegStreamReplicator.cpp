...
class JpegStreamReplica: public MjpegLiveVideoSource {
protected:
  friend class JpegStreamReplicator;
  JpegStreamReplica(JpegStreamReplicator& ourReplicator); // called only by "StreamReplicator::createStreamReplica()"
  virtual ~JpegStreamReplica();
...
private:
  static void copyReceivedFrame(JpegStreamReplica* toReplica, JpegStreamReplica* fromReplica);

private:
  JpegStreamReplicator& fOurReplicator;
  ...
  JpegStreamReplica* fNext;
};


////////// StreamReplicator implementation //////////

JpegStreamReplicator* JpegStreamReplicator::createNew(UsageEnvironment& env, MjpegLiveVideoSource* inputSource, Boolean deleteWhenLastReplicaDies) {
  return new JpegStreamReplicator(env, inputSource, deleteWhenLastReplicaDies);
}

JpegStreamReplicator::JpegStreamReplicator(UsageEnvironment& env, MjpegLiveVideoSource* inputSource, Boolean deleteWhenLastReplicaDies)
  : Medium(env),
...
JpegStreamReplicator::~JpegStreamReplicator() {
  Medium::close(fInputSource);
}

MjpegLiveVideoSource* JpegStreamReplicator::createStreamReplica() {
  // std::cout << "JpegStreamReplicator::createStreamReplica called" << std::endl;
  ++fNumReplicas;
  return new JpegStreamReplica(*this);
}

void JpegStreamReplicator::getNextFrame(JpegStreamReplica* replica) {
  // std::cout << "JpegStreamReplicator::getNextFrame() called" << std::endl;
...
void JpegStreamReplicator::deactivateStreamReplica(JpegStreamReplica* replicaBeingDeactivated) {
  ...
  if (replicaBeingDeactivated == fPrimaryReplica) {
    ...
    if (fPrimaryReplica != NULL) {
      JpegStreamReplica::copyReceivedFrame(fPrimaryReplica, replicaBeingDeactivated);
    } else {
    ...
    for (JpegStreamReplica* r1 = fReplicasAwaitingCurrentFrame; r1->fNext != NULL; r1 = r1->fNext) {
    ...
    for (JpegStreamReplica* r2 = fReplicasAwaitingNextFrame; r2->fNext != NULL; r2 = r2->fNext) {
...
void JpegStreamReplicator::removeStreamReplica(JpegStreamReplica* replicaBeingRemoved) {
...
void JpegStreamReplicator::afterGettingFrame(void* clientData, unsigned frameSize, unsigned numTruncatedBytes,
...
void JpegStreamReplicator::afterGettingFrame(unsigned frameSize, unsigned numTruncatedBytes,
                     struct timeval presentationTime, unsigned durationInMicroseconds) {
...
void JpegStreamReplicator::onSourceClosure(void* clientData) {
  ((JpegStreamReplicator*)clientData)->onSourceClosure();
}

void JpegStreamReplicator::onSourceClosure() {
  fInputSourceHasClosed = True;

  // Signal the closure to each replica that is currently awaiting a frame:
  JpegStreamReplica* replica;
...
void JpegStreamReplicator::deliverReceivedFrame() {
  ...
  while ((replica = fReplicasAwaitingCurrentFrame) != NULL) {
    ...
    if (fPrimaryReplica == NULL) fprintf(stderr, "JpegStreamReplicator::deliverReceivedFrame() Internal Error 1!\n"); // shouldn't happen
    JpegStreamReplica::copyReceivedFrame(replica, fPrimaryReplica);
    ...
    if (!(fNumDeliveriesMadeSoFar < fNumActiveReplicas)) fprintf(stderr, "JpegStreamReplicator::deliverReceivedFrame() Internal Error 2(%d,%d)!\n", fNumDeliveriesMadeSoFar, fNumActiveReplicas); // should not happen
...
JpegStreamReplica::JpegStreamReplica(JpegStreamReplicator& ourReplicator)
  : MjpegLiveVideoSource(ourReplicator.envir()),
    fOurReplicator(ourReplicator),
    fFrameIndex(-1/*we haven't started playing yet*/), fNext(NULL) {
    // std::cout << "JpegStreamReplicator::createStreamReplica called" << std::endl;
}

JpegStreamReplica::~JpegStreamReplica() {
  fOurReplicator.removeStreamReplica(this);
}

void JpegStreamReplica::doGetNextFrame() {
  // std::cout << "JpegStreamReplica::doGetNextFrame() called" << std::endl;
  fOurReplicator.getNextFrame(this);

  // update special parameters for jpeg
  this->fParser = fOurReplicator.inputSource()->Parser();
}

void JpegStreamReplica::doStopGettingFrames() {
  fOurReplicator.deactivateStreamReplica(this);
}

void JpegStreamReplica::copyReceivedFrame(JpegStreamReplica* toReplica, JpegStreamReplica* fromReplica) {

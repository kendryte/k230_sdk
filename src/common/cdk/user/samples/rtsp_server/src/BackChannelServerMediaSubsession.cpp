...
void StreamState::reclaim() {
  ...
  if(fDataSink) fMaster.closeStreamSink(fDataSink); fDataSink = NULL;
  if(fRTPSource) Medium::close(fRTPSource); fRTPSource = NULL;
  if(fUDPSource) Medium::close(fUDPSource); fUDPSource = NULL;

...
class UserDataFifoWriter::Impl {
  public:
    ...
    int Init(int item_size, int item_count, uint64_t &phy_addr, IDataFifoRelease *func) {
        ...
        k_s32 s32Ret = kd_datafifo_open(&handle_, &datafifo_params);
        if (K_SUCCESS != s32Ret) {
            printf("%s open datafifo error:%x\n",__FUNCTION__,s32Ret);
            return -1;
        }
        s32Ret = kd_datafifo_cmd(handle_, DATAFIFO_CMD_GET_PHY_ADDR, &phy_addr);
        if (K_SUCCESS != s32Ret) {
    ...
    void DeInit() {
        ...
        if (handle_) {
            ...
            k_s32 s32Ret = kd_datafifo_write(handle_, NULL);
            if (K_SUCCESS != s32Ret) {

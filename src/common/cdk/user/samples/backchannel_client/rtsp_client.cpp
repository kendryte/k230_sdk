...
void openURL(UsageEnvironment& env, char const* progName, char const* rtspURL) {
  ...
  ++rtspClientCount;
  rtspClient->bRequireBackChannel = true;
...
int KdRtspClient::Init(IOnAudioData *on_audio_data, IRtspClientEvent *on_event) {
    scheduler_ = BasicTaskScheduler::createNew();
    env_ = BasicUsageEnvironment::createNew(*scheduler_);
    g711_source_ = G711LiveFrameSource::createNew(*env_, 8);
    on_audio_data_ = on_audio_data;
    on_event_ = on_event;
    return 0;
}
...
int KdRtspClient::SendAudioData(const uint8_t *data, size_t size, uint64_t timestamp) {
    if (g711_source_) {
        g711_source_->pushData(data, size, timestamp);
        return 0;
    }
    return -1;
}

...
RTSPClient::RTSPClient(UsageEnvironment& env, char const* rtspURL,
  ...
  : Medium(env),
    ...
    fTLS(*this), fPOSTSocketTLS(*this),bRequireBackChannel(False) {
...
Boolean RTSPClient::setRequestFields(RequestRecord* request,
  ...
  if (strcmp(request->commandName(), "DESCRIBE") == 0) {
    ...
    extraHeaders = (char*)( bRequireBackChannel == True ? "Accept: application/sdp\r\nRequire: www.onvif.org/ver20/backchannel\r\n" : "Accept: application/sdp\r\n");
...
Boolean RTSPClient::handleSETUPResponse(MediaSubsession& subsession, char const* sessionParamsStr, char const* transportParamsStr,
  ...
  do {
	...
	increaseReceiveBufferTo(envir(), fInputSocketNum, 50*1024);
      ...
      // for audio back channel
      if (bRequireBackChannel == True && subsession.sink != NULL) {
        RTPSink *sink = (RTPSink *)(subsession.sink);
        sink->setStreamSocket(fInputSocketNum, subsession.rtpChannelId, fInputTLS);
        increaseReceiveBufferTo(envir(), fInputSocketNum, 50*1024);
      }
...
void RTSPClient::connectionHandler1() {
  ...
  do {
    ...
    if (getsockopt(fOutputSocketNum, SOL_SOCKET, SO_ERROR, (char*)&err, (socklen_t *)&len) < 0 || err != 0) {

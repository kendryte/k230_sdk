...
shrink_rootfs_doorlock_ov9286()
{
	...
	#小核裁剪
	cp  ${K230_SDK_ROOT}/board/k230_evb_doorlock/doorlock_ov9286_rcS  ${BUILD_DIR}/images/little-core/rootfs/etc/init.d/rcS;
	...
	cd ${BUILD_DIR}/images/big-core/root/bin/; 
		...
		cp ${K230_SDK_ROOT}/src/reference/business_poc/doorlock_ov9286/big/out/door_lock.elf  fastboot_app.elf;
		...
		find . -type f  -not -name fastboot_app.elf -not -name init.sh -not -name H1280W720_conf.bin -not -name H1280W720_ref.bin \
				-not -name mbface.kmodel -not -name retinaface.kmodel -not -name ir.kmodel -not -name depth.kmodel | xargs rm -rf ;
